...
async function GetNotInstalledBuckets() {
    // function GetNotInstalledBuckets() {
        try {
        return await fs.readFile(bucketList, { encoding: "utf-8" }).then(async (e) => {
                let bucketsList = Object.keys(JSON.parse(e)); // parse all the known buckets
                return await ParseFiles(`${process.env.USERPROFILE}\\scoop\\buckets`).then((instBuckets) => {
...
// module.exports = { test, ParseFiles, GetNotInstalledBuckets, read, test2 };
export { test, ParseFiles, GetNotInstalledBuckets, read, test2, path, fs, bucketList, bucketPath, BucketNames, appsPath, sweetPromise}
